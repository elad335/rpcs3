...
spu_program spu_recompiler_base::analyse(const be_t<u32>* ls, u32 entry_point, std::map<u32, std::vector<u32>>* out_target_list)
{
	...
	const auto initiate_patterns = [&](block_reg_state_iterator& block_state_it, u32 bpc)
	{
	...
	for (u32 wf = 0, wi = 0, wa = entry_point, bpc = wa; wf <= 1;)
	{
		...
		const auto reduced_loop = is_pattern_match ? &::at32(reg_state_it, wi).reduced_loop : &dummy_rloop;
		...
		const auto next_block = [&]()
		{
			...
			else
			{
				...
				if (insert_entry)
				{
					...
					initiate_patterns(next, target_pc);
			...
			if (wi >= reg_state_it.size())
			{
				...
				if (wf == 1)
				{
					...
					if (!infos.empty())
					{
						...
						initiate_patterns(reg_state_it.back(), ::at32(infos, entry_point)->pc);
					}
				}
			}
			...
			while (reduced_loop->active)
			{
				...
				for (u32 i = 0; i < reg->regs.size() && reduced_loop->active; i++)
				{
					if (reg->regs.test(i))
					{
						...
						const auto reg_org = reduced_loop->find_reg(i);

						if (!reg_org || reg_org->modified == 0)
						{
						...
						else
						{
							...
							const u32 loop_arg_reg = i == op_ra ? op_rb : op_ra;
							const u32 loop_dict_reg = i == op_ra ? op_ra : op_rb;
							reduced_loop->cond_val_is_immediate = false;
	...
	for (const auto& [loop_pc, pattern] : reduced_loop_all)
	{
		...
		if (inst_attr attr = m_inst_attrs[(loop_pc - entry_point) / 4]; attr == inst_attr::none)
		{
			...
			spu_log.success("Reduced Loop Pattern Detected! (REGS: %s, DICT: r%d, ARG: %s, Incr: %d (%s), CMP/Size: %s/%u, loop_pc=0x%x, 0x%x-%s)", regs, pattern.cond_val_register_idx
				, pattern.cond_val_is_immediate ? fmt::format("0x%x", pattern.cond_val_min) : fmt::format("r%d", pattern.cond_val_register_argument_idx)
				, static_cast<s32>(pattern.cond_val_incr), pattern.cond_val_incr_before_cond ? "BEFORE" : "AFTER"
