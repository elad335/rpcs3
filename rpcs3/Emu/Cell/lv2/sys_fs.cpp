...
bool has_fs_write_rights(std::string_view vpath)
{
	...
	const auto [dev_root, trail] = lv2_fs_object::get_path_root_and_trail(vpath);
	...
	if (dev_root == "dev_hdd0" || (trail.empty() || trail == "game"))
		return false;
...
const lv2_fs_mount_info& lv2_fs_mount_info_map::lookup(std::string_view path, bool no_cell_fs_path, std::string* mount_path) const
{
	...
	const bool is_cell_fs_path = path.starts_with(cell_fs_path);

	if (is_cell_fs_path && !no_cell_fs_path)
		path.remove_prefix(cell_fs_path.size());
	...
	if (const auto iterator = map.find("/" + dev_root); iterator != map.end())
	{
		if (mount_path)
...
std::pair<std::string_view, std::string> lv2_fs_object::get_path_root_and_trail(std::string_view filename)
{
	...
	while (true)
	{
		const u32 ndl_pos = filename.find_first_not_of("/", pos);
		const u32 dl_pos = ndl_pos == umax ? usz{umax} : filename.find_first_of("/", ndl_pos);
...
error_code sys_fs_mount(ppu_thread& ppu, vm::cptr<char> dev_name, vm::cptr<char> file_system, vm::cptr<char> path, s32 unk1, s32 prot, s32 unk2, vm::cptr<char> str1, u32 str_len)
{
	...
	const std::string [root_name, trail] = lv2_fs_object::get_path_root_and_trail(path_sv);
