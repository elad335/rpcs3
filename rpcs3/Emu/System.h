...
enum class system_state : u32
{
	...
	stopped,
	paused,
	fatal, // paused but cannot resume
	ready,
};
...
public:
	...
	bool Pause(bool no_resume = false);
	void Resume();
	...
	auto GetStatus() const { system_state state = m_state; return state == system_state::fatal ? system_state::paused : state; }
